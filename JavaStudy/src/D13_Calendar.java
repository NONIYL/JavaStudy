...
public class D13_Calendar {
	...
	public static void main(String[] args) {
		...
		//번호만으로는 필드를 외울 수 없기 때문에, 캘린더 클래스에 상수가 만들어져있다
		...
		//이 YEAR에는 final public static int를 유추할 수 있다 
		...
		System.out.println("이번달이 몇 번째 주인가?" + now.get(Calendar.DAY_OF_WEEK_IN_MONTH));
		System.out.println("이번달이 몇 번째 달인가?" + now.get(Calendar.WEEK_OF_MONTH));
		System.out.println("이번달이 몇 번째 날인가?" + now.get(Calendar.DAY_OF_YEAR));
		//요일을 알 수 있지만 번호로 나와서 알기 힘들다
		System.out.println("이번달 몇 번째 날인가(요일)?" +  now.get(Calendar.DAY_OF_WEEK));
		...
		System.out.println("12시: " + now.get(Calendar.HOUR_OF_DAY));
		...
		//set(fieldNumber, value) : 필드번호로 필드를 선택하여 인스턴스의 특정 필드의 값을 변경하는 메서드
		...
		//getDisplayName(field, style,) : 달, 요일 등을 번호 대신 보기 좋은 문자 이름으로 꺼낸다
		//style - 길게 표현할지 짧게 표현할지 등을 결정한다
		//Locale - 어떤 언어로 표현할지 결정한다
		String dayOfWeek = now.getDisplayName(
		...
		/*
		 	...
		 	- Date 타입 인스터스를 편리하게 출력하는 기능이 구현되어 있는 클래스
		...
		//시간이 몇초만 달라도 equals가 false이기 때문에 같은 날짜인지 구분하기 위해 equlse를 쓰는 것은 불가능하다 
