...
public class A05_TypeCasting {
	...
	/* 
	  ...
	  byte < short, char < int < long(8) < float(4) < double
	   ...
	   - 크기가 더 큰 타입의 변수에 작은 크기의 값을 넣을떄는 타입이 자동으로 변환된다
	    (자연스러운 타입 캐스팅)
	...
	*/
      ...
      public static void main(String[] arge) {
    	  ...
    	  // 큰 타입 값을 작은 타입에 넣을 때는 강제 타입 캐스팅으로 대입이 가능하지만
          // 값이 손상될 수도 있다
          
          // 바이트 양수 최댓값 +127까지 만약 그 이상으로 하려하면 오버플로우 발생으로 오류 숫자 발생
    	  int num = 200;
    	  byte bnum = (byte)num;   
    	  // 큰 숫자 감당할꼐 붙여봐 ...-> (byte) -> 근데 값이 손상될 수 있음;
    	 ...
    	 // char에 short값을 넣을 때는 음수값이 존재할 위험이 있으므로 강제형변환이 필요하다
    	    ...
    	    // short에 char값을 넣을 때는 32767을 초과하는 양수가 존재할 위험이 있다(-도 초과로 봄)
    	    snum = (short)ch1;
