...
public class C13_AbstractClass {
	...
	/*
	  ...
	  - 미완성 메서드를 적어 놓고 나중에 상속받아서 마저 만들어야 하는 클래스
	   - 추상 클래스 내부에는 완성되지 않은 메서드가 들어있으므로 인스턴스를 생성할 수 없다
	  
	  # 추상 메서드 (abstract method)
	  
	  - 추상 클래스 내부에 들어있는 미완성 메서드 
	  - 선언만 해놓고 구현은 상속받은 자식들에게 미룬다 
	  - 오버라이드하지 않으면 컴파일 에러가 발생하여 자식 클래스에게 오버라이딩을 강제하는 효과가 있다
...
abstract class Crop {
	...
	final int harvest() {
		int qty = this.qty; // this.qty에 qty것 을 보관
		this.qty = 0; // 이걸 0으로 만들어줘
		return qty; // 그럼 수확 후 0으로 만들 수 있음?????? 뭣ㄴ소리야시발
	}
}
