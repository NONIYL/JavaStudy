...
public class C04_static {
	...
	/*
	  ...
	  -스태틱 영역은 인스턴스가 하나도 생기지 않은 시점에도 (스택틱 영역은) 존재한다 (최초로 해당 클래스가 호출되었을 때 생성된다) 
	 ...
	 - 스태틱 메서드 (인스턴스가 하나도 존재하지 때도 동작할 수 있는 메서드-> 인스턴스를 활용하지 않는 메서드) 
	 - 클래스 내부의 스태틱 클래스 
	 - 그동안 썼던 Math.random은 스태틱 함수였음
	  
	 */

	public static void main(String[] args) {
		// 인스턴스를 하나도 생성하지 않은 시점에도 static 자원을 활용할 수 있다
		System.out.println(TrumphCard.card_width); // 80
		System.out.println(TrumphCard.card_height); // 150
...
class TrumphCard {
	...
	// 카드의 너비와 높이는 몯느 인스턴스에서 다를 필요가 없는 값이다
	// 이런 성질의 변수를 인스턴스 값으로 두면 메모리의 공간이 낭비된다
	// 그러니까 static을 달아두면 낭비를 막죠?
	static int card_width = 80; // static의 영역을 this의 영역에 넣을 수 있음,이미 태초에 생겨나있기 때문..에?..?
