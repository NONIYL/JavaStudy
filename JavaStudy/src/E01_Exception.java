...
public class E01_Exception {
	...
	/*
	 	...
	 	- 예외가 발생했을때 예외처리가 되어있지 않은 경우 프로그램이 강제종료 되지만
	 	  해당 예외에 대한 처리가 되어있는 경우 프로그램이 강제종료되지 않는다 
	 	- 예외가 발생하여 해당 catch문에 도착할 때 발생한 예외에 대한 정보를 담고있는 인스턴스가 함께 도착한다
	...
	public static void main(String[] args) {
		try {
			...
			//System.out.println(e.getMessage()); // size = 10 / '0;' 요부분 
		}
		
		//스택(함수 호출 순서를 기억하기 위한 자료구조
		//스택 트레이스, 스택 오버플로우
		try {
		...
		try {
			int[] nums = new int[3];
			nums[(int)(Math.random() * 5)] = 7; //랜덤으로 예외가 발생했다는 문구가 나옴
			...
			//Exception은 모든 예외객체의 조상 클래스이기 때문에 모든 예외가 이 catch문으로 도착할 수 있다
			System.out.println("예외가 발생했습니다!"); 
//			System.out.println(" /)/)");
//			System.out.println("(  ..)");
//			System.out.println("( >@ )");
			//만약, 예외가 발생해도 내가 토끼를 출력하고 싶다면 여기에 토끼를 프린트해준다 
		} finally {
