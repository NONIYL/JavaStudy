...
public class E12_NetworkProgramming {
	/*
	 	...
	 	- 서버 소켓이 접속되고 나면 양방향으로 통신이 가능하다
	 	- 서로 원하는 데이터를 전송할 수 있다
	 	- 상대방에게 전송하는 것이 Output이 되고 , 상대방으로부터 받는 것이 Input이 된다 
	 	
	 	# 네트워크 기본지식
	 	
	 	- IP주소 : 0.0.0.0 ~ 255.255.255.255 사시의 번호로 이루어진 컴퓨터의 주소
	 	※ IP주소는 cmd -> ipconfig 명령어로 체크할 수 있다
	 	- 프로토콜(Protocol): 통신규칙(TCP, UDP, SMTP,Telnet, ftp, http...)
	 	- 포트번호: 하나의 컴퓨터에서 각 프로그램을 구분하기 위한 용도로 사용되는 번호 
	 		※ 현재 컴퓨터에서 열려있는 포드 번호는 cmd - > netstat 명령어로 확인 할 수 있다
	...
	public static void main(String[] args) {
		...
		try (
			...
			System.out.println("우리서버의 IP주소" + ss.getLocalSocketAddress());
			System.out.println("우리서버의 포트번호: " + ss.getLocalPort());
