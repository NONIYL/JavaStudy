...
public class D01_ArrayList {
	/*
	 	...
	 	- 데이터를 가장 효율적으로 다루기 위해 어떻게 해야 하는가를 다루는 학문
	 	 ex> 수 많은 데이터 중 '특정 데이터'를 찾기 위해서는 어떻게 하는게 가장 효율적일까?
	 	 	 수 많은 데이터들을 '순서대로 재배치'하는데 가장 효율적인 방법은 무엇일까?
	...
	public static void main(String[] args) {
		...
		//어떤 타입으로 꺼내야 할지 모르는 경우 object타입으로 받는다 
		...
		//list.size(): 해당 리스트의 크기를 반환한다 (len과 비슷한 의미를 지님)
		...
		//정확히 무슨 값으로 받는다고 안하면 object타입으로 받아서 계산x  
//		System.out.println((int)list.get(4) + (double)list.get(5));
		...
		//기본형 타입을 제네릭으로 사용하기 위해서는 참조형 버전을 사용해야한다
		...
		System.out.println("삭제에 성공한 겂: " + deleted);
		System.out.println(scores);
		
		//list.remove(value): 해당 값을 삭제한다. 일치하는 값이 여러개면 맨 앞에 있는 값 하나만 지운다(성공 여부를 반환한다)
		// ※ int의 경우 값으로 삭제하고 싶으면 Integer.valueOf() 메서드를 사용해야한다
		...
		//list.set(index, value) : 해당 번째 값을 원하는 값으로 수정한다(수정하면서 수정전 원하는 값을 반환해준다)
		...
		//list.subList(from, to) : 리스트의 일부분을 새로운 리스트로 만들어 반환 (frome 이상 to미만)
		List<String> sub1 = democracy.subList(0, 3); // 0~2
		List<String> sub2 = democracy.subList(3, 6); // 3~5
		...
		//list.removeAll(list2): list1에 list2의 포함된 모든 값을 삭제한다
		num1.removeAll(num2);
		System.out.println(num1);
		...
		//list.retainAll(list2): list1과 list2에 모두 포함된 값만 남긴다
		num1.retainAll(num2);
		System.out.println(num1);
