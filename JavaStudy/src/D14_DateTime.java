...
public class D14_DateTime {
	...
	public static void main(String[] args) {
		...
		System.out.println(now_dt); //현재날짜, 시간.초
		System.out.println(now_d); // 현재 날짜
		System.out.println(now_t); //현재시간.초
		...
		System.out.println(dow.getDisplayName(TextStyle.SHORT, Locale.ITALIAN));
		
		//7가지 요일인스턴스는 DayOfWeek 클래스의 상수로 만들어져있다
		...
		System.out.println(now_dt.getDayOfMonth());
		...
		System.out.println(LocalDate.of(2001, 5, 13)); //년,월,일
		System.out.println(LocalTime.of(14, 36)); // 시,분,초
		System.out.println(LocalDateTime.of(date1, time1));
		
		// with() : 해당 인스턴스의 특정 필드를 수정한 인스턴스를 반환한다
		LocalDate date2 = date1.withMonth(7);
		LocalDate date3 = date2.plusMonths(7);
		
		System.out.println(date2);
		System.out.println(date3);
		
		//시간의 단위(TemporalField)는 ChoronoField 클래스의 상수로 정의되어 있다
		...
		// TemporalAdjusters 클래스 :  TempralAdjusters를 구현한 인스턴스들을 모아놓은 클래스
		...
		System.out.println(childrensDay.with(TemporalAdjusters.lastDayOfMonth()));
