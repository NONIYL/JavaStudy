...
public class B13_Array {
	...
	public static void main(String[] args) { 
		...
		numbers[4] = 'A'; // 문자열이 들어가는ㄴ이유: 숫자이기도 하지만 int값에 들어가는 bity라서
...
//		//값을 한번도 넣은적 없는 변수에는 0이 자동으로 들어가 있지 않다. 사용할 수 없다
//		int a, b, c;
//		
...
//		
		...
		double[] avgs = new double[] {
				...
				'김', 453545345745L, 123456, 3.1234F }; 
		
		//배열은 그냥 출력하면 내용이 제대로 출력되지 않고 메모리상의 위치가 출력된다
		//예외로 char[]은 문자열 취급을 받아 내용이 (문자열처럼) 출력된다
		System.out.println(nums); //메모리상 배열 출력값
		System.out.println(message);
		System.out.println(cmds);//메모리상 배열 출력값
		System.out.println(urls);
		System.out.println(avgs[8]); // (avgs)랑 값이 다른걸 확인할 수 있음
