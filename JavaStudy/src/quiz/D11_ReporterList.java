...
public class D11_ReporterList {
	...
	/*
	  1. 어떤 이메일 도메인 (yna.co.kr, hankyung.com 등) 이 몇 개씩 있는지 세어서 출력 
	  2. 어떤 신문사의 기자가 몇명씩 있는지 세어서 출력 
...
public static void main(String[] args) {
		...
		ArrayList<Reporter> reportersList = new ArrayList<>(); // Reporter 파일 불러옴
		...
		while (m.find()) {
			...
			//▼ 생성자						//이름 	//회사m.group(1)	//분야m.group(2)	//이메일
			reportersList.add(new Reporter(m.group(3), company, field, m.group(4)));
		...
		//Map: 키와 값을 저장할 수 있는것 
		//categoryComanyMap: 키값으로 회사의 이름을 쓰고 값으로 리스트를 사용한다
		...
		//reportersList: 정규표현식으로 찾은 기자들은 모두 더해놓은 리스트
		//forEach: 하나씩 꺼내면서 반복
		for (Reporter reporter : reportersList) {
			...
			if(categoryCompany.containsKey(companyName)) {
				...
				//리스트인데 reporter만 담을 수 있는 list.
				ArrayList<Reporter> list = new ArrayList<>();
				//리포터만 더하겠다
				list.add(reporter);
				//list = reporter. 리스트에 리포터를 담겠다
				categoryCompany.put(companyName, list);
			}
		}
		System.out.println("[[[[[[언론사별 기자 리스트]]]]]]]");
		...
		//숙제 5번 풀기.
		HashMap<String, List<Reporter>> categoryFieldMap = new HashMap<>();
